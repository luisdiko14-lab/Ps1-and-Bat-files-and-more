<#
.SYNOPSIS
  Utility script 214 for lightweight workstation automation.
.DESCRIPTION
...
function DisableJsonSettings214 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '214'
        ScriptName = 'DisableJsonSettings214'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'DisableJsonSettings214.log'
...
$result = DisableJsonSettings214 -Destination $OutputPath
