<#
.SYNOPSIS
  Utility script 211 for lightweight workstation automation.
.DESCRIPTION
...
function StartJsonSettings211 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '211'
        ScriptName = 'StartJsonSettings211'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'StartJsonSettings211.log'
...
$result = StartJsonSettings211 -Destination $OutputPath
