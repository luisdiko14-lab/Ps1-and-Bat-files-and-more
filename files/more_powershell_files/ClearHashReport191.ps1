<#
.SYNOPSIS
  Utility script 191 for lightweight workstation automation.
.DESCRIPTION
...
function ClearHashReport191 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '191'
        ScriptName = 'ClearHashReport191'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ClearHashReport191.log'
...
$result = ClearHashReport191 -Destination $OutputPath
