<#
.SYNOPSIS
  Utility script 102 for lightweight workstation automation.
.DESCRIPTION
...
function SetNetworkLatency102 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '102'
        ScriptName = 'SetNetworkLatency102'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'SetNetworkLatency102.log'
...
$result = SetNetworkLatency102 -Destination $OutputPath
