<#
.SYNOPSIS
  Utility script 011 for lightweight workstation automation.
.DESCRIPTION
...
function StartDiskHealth011 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '011'
        ScriptName = 'StartDiskHealth011'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'StartDiskHealth011.log'
...
$result = StartDiskHealth011 -Destination $OutputPath
