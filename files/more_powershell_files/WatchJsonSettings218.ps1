<#
.SYNOPSIS
  Utility script 218 for lightweight workstation automation.
.DESCRIPTION
...
function WatchJsonSettings218 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '218'
        ScriptName = 'WatchJsonSettings218'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'WatchJsonSettings218.log'
...
$result = WatchJsonSettings218 -Destination $OutputPath
