<#
.SYNOPSIS
  Utility script 199 for lightweight workstation automation.
.DESCRIPTION
...
function RestoreHashReport199 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '199'
        ScriptName = 'RestoreHashReport199'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'RestoreHashReport199.log'
...
$result = RestoreHashReport199 -Destination $OutputPath
