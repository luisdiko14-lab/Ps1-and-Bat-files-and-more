<#
.SYNOPSIS
  Utility script 162 for lightweight workstation automation.
.DESCRIPTION
...
function StopFolderInventory162 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '162'
        ScriptName = 'StopFolderInventory162'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'StopFolderInventory162.log'
...
$result = StopFolderInventory162 -Destination $OutputPath
