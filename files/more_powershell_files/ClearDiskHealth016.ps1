<#
.SYNOPSIS
  Utility script 016 for lightweight workstation automation.
.DESCRIPTION
...
function ClearDiskHealth016 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '016'
        ScriptName = 'ClearDiskHealth016'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ClearDiskHealth016.log'
...
$result = ClearDiskHealth016 -Destination $OutputPath
