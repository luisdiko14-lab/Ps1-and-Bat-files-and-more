<#
.SYNOPSIS
  Utility script 075 for lightweight workstation automation.
.DESCRIPTION
...
function FormatTopProcesses075 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '075'
        ScriptName = 'FormatTopProcesses075'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'FormatTopProcesses075.log'
...
$result = FormatTopProcesses075 -Destination $OutputPath
