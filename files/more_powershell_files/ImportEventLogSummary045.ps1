<#
.SYNOPSIS
  Utility script 045 for lightweight workstation automation.
.DESCRIPTION
...
function ImportEventLogSummary045 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '045'
        ScriptName = 'ImportEventLogSummary045'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ImportEventLogSummary045.log'
...
$result = ImportEventLogSummary045 -Destination $OutputPath
