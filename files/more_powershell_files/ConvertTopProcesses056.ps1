<#
.SYNOPSIS
  Utility script 056 for lightweight workstation automation.
.DESCRIPTION
...
function ConvertTopProcesses056 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '056'
        ScriptName = 'ConvertTopProcesses056'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ConvertTopProcesses056.log'
...
$result = ConvertTopProcesses056 -Destination $OutputPath
