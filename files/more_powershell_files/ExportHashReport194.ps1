<#
.SYNOPSIS
  Utility script 194 for lightweight workstation automation.
.DESCRIPTION
...
function ExportHashReport194 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '194'
        ScriptName = 'ExportHashReport194'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ExportHashReport194.log'
...
$result = ExportHashReport194 -Destination $OutputPath
