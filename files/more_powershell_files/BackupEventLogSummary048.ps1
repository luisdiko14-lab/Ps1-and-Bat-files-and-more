<#
.SYNOPSIS
  Utility script 048 for lightweight workstation automation.
.DESCRIPTION
...
function BackupEventLogSummary048 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '048'
        ScriptName = 'BackupEventLogSummary048'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'BackupEventLogSummary048.log'
...
$result = BackupEventLogSummary048 -Destination $OutputPath
