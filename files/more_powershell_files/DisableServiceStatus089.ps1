<#
.SYNOPSIS
  Utility script 089 for lightweight workstation automation.
.DESCRIPTION
...
function DisableServiceStatus089 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '089'
        ScriptName = 'DisableServiceStatus089'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'DisableServiceStatus089.log'
...
$result = DisableServiceStatus089 -Destination $OutputPath
