<#
.SYNOPSIS
  Utility script 185 for lightweight workstation automation.
.DESCRIPTION
...
function NewHashReport185 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '185'
        ScriptName = 'NewHashReport185'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'NewHashReport185.log'
...
$result = NewHashReport185 -Destination $OutputPath
