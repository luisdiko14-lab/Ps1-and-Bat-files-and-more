<#
.SYNOPSIS
  Utility script 150 for lightweight workstation automation.
.DESCRIPTION
...
function FormatUptime150 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '150'
        ScriptName = 'FormatUptime150'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'FormatUptime150.log'
...
$result = FormatUptime150 -Destination $OutputPath
