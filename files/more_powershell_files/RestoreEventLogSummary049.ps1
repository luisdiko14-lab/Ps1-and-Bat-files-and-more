<#
.SYNOPSIS
  Utility script 049 for lightweight workstation automation.
.DESCRIPTION
...
function RestoreEventLogSummary049 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '049'
        ScriptName = 'RestoreEventLogSummary049'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'RestoreEventLogSummary049.log'
...
$result = RestoreEventLogSummary049 -Destination $OutputPath
