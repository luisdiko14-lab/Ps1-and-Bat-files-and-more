<#
.SYNOPSIS
  Utility script 169 for lightweight workstation automation.
.DESCRIPTION
...
function ExportFolderInventory169 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '169'
        ScriptName = 'ExportFolderInventory169'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ExportFolderInventory169.log'
...
$result = ExportFolderInventory169 -Destination $OutputPath
