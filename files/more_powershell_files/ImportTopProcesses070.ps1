<#
.SYNOPSIS
  Utility script 070 for lightweight workstation automation.
.DESCRIPTION
...
function ImportTopProcesses070 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '070'
        ScriptName = 'ImportTopProcesses070'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ImportTopProcesses070.log'
...
$result = ImportTopProcesses070 -Destination $OutputPath
