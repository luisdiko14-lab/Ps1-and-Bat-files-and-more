<#
.SYNOPSIS
  Utility script 008 for lightweight workstation automation.
.DESCRIPTION
...
function WriteDiskHealth008 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '008'
        ScriptName = 'WriteDiskHealth008'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'WriteDiskHealth008.log'
...
$result = WriteDiskHealth008 -Destination $OutputPath
