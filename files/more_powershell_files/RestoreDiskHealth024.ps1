<#
.SYNOPSIS
  Utility script 024 for lightweight workstation automation.
.DESCRIPTION
...
function RestoreDiskHealth024 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '024'
        ScriptName = 'RestoreDiskHealth024'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'RestoreDiskHealth024.log'
...
$result = RestoreDiskHealth024 -Destination $OutputPath
