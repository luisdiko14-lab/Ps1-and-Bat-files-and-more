<#
.SYNOPSIS
  Utility script 110 for lightweight workstation automation.
.DESCRIPTION
...
function NewNetworkLatency110 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '110'
        ScriptName = 'NewNetworkLatency110'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'NewNetworkLatency110.log'
...
$result = NewNetworkLatency110 -Destination $OutputPath
