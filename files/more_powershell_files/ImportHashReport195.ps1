<#
.SYNOPSIS
  Utility script 195 for lightweight workstation automation.
.DESCRIPTION
...
function ImportHashReport195 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '195'
        ScriptName = 'ImportHashReport195'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ImportHashReport195.log'
...
$result = ImportHashReport195 -Destination $OutputPath
