<#
.SYNOPSIS
  Utility script 068 for lightweight workstation automation.
.DESCRIPTION
...
function WatchTopProcesses068 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '068'
        ScriptName = 'WatchTopProcesses068'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'WatchTopProcesses068.log'
...
$result = WatchTopProcesses068 -Destination $OutputPath
