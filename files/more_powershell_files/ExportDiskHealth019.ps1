<#
.SYNOPSIS
  Utility script 019 for lightweight workstation automation.
.DESCRIPTION
...
function ExportDiskHealth019 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '019'
        ScriptName = 'ExportDiskHealth019'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ExportDiskHealth019.log'
...
$result = ExportDiskHealth019 -Destination $OutputPath
