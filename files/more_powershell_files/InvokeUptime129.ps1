<#
.SYNOPSIS
  Utility script 129 for lightweight workstation automation.
.DESCRIPTION
...
function InvokeUptime129 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '129'
        ScriptName = 'InvokeUptime129'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'InvokeUptime129.log'
...
$result = InvokeUptime129 -Destination $OutputPath
