<#
.SYNOPSIS
  Utility script 229 for lightweight workstation automation.
.DESCRIPTION
...
function InvokeCsvCleanup229 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '229'
        ScriptName = 'InvokeCsvCleanup229'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'InvokeCsvCleanup229.log'
...
$result = InvokeCsvCleanup229 -Destination $OutputPath
