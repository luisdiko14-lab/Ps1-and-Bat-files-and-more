<#
.SYNOPSIS
  Utility script 021 for lightweight workstation automation.
.DESCRIPTION
...
function CompareDiskHealth021 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '021'
        ScriptName = 'CompareDiskHealth021'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'CompareDiskHealth021.log'
...
$result = CompareDiskHealth021 -Destination $OutputPath
