<#
.SYNOPSIS
  Utility script 007 for lightweight workstation automation.
.DESCRIPTION
...
function UpdateDiskHealth007 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '007'
        ScriptName = 'UpdateDiskHealth007'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'UpdateDiskHealth007.log'
...
$result = UpdateDiskHealth007 -Destination $OutputPath
