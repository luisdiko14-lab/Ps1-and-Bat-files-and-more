<#
.SYNOPSIS
  Utility script 046 for lightweight workstation automation.
.DESCRIPTION
...
function CompareEventLogSummary046 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '046'
        ScriptName = 'CompareEventLogSummary046'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'CompareEventLogSummary046.log'
...
$result = CompareEventLogSummary046 -Destination $OutputPath
