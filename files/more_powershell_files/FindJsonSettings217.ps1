<#
.SYNOPSIS
  Utility script 217 for lightweight workstation automation.
.DESCRIPTION
...
function FindJsonSettings217 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '217'
        ScriptName = 'FindJsonSettings217'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'FindJsonSettings217.log'
...
$result = FindJsonSettings217 -Destination $OutputPath
