<#
.SYNOPSIS
  Utility script 066 for lightweight workstation automation.
.DESCRIPTION
...
function ClearTopProcesses066 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '066'
        ScriptName = 'ClearTopProcesses066'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ClearTopProcesses066.log'
...
$result = ClearTopProcesses066 -Destination $OutputPath
