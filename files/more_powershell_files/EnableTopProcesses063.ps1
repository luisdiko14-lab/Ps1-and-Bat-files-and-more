<#
.SYNOPSIS
  Utility script 063 for lightweight workstation automation.
.DESCRIPTION
...
function EnableTopProcesses063 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '063'
        ScriptName = 'EnableTopProcesses063'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'EnableTopProcesses063.log'
...
$result = EnableTopProcesses063 -Destination $OutputPath
