<#
.SYNOPSIS
  Utility script 142 for lightweight workstation automation.
.DESCRIPTION
...
function FindUptime142 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '142'
        ScriptName = 'FindUptime142'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'FindUptime142.log'
...
$result = FindUptime142 -Destination $OutputPath
