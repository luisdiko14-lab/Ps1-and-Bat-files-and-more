<#
.SYNOPSIS
  Utility script 200 for lightweight workstation automation.
.DESCRIPTION
...
function FormatHashReport200 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '200'
        ScriptName = 'FormatHashReport200'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'FormatHashReport200.log'
...
$result = FormatHashReport200 -Destination $OutputPath
