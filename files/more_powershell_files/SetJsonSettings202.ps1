<#
.SYNOPSIS
  Utility script 202 for lightweight workstation automation.
.DESCRIPTION
...
function SetJsonSettings202 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '202'
        ScriptName = 'SetJsonSettings202'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'SetJsonSettings202.log'
...
$result = SetJsonSettings202 -Destination $OutputPath
