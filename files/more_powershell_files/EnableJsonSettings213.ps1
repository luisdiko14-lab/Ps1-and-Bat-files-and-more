<#
.SYNOPSIS
  Utility script 213 for lightweight workstation automation.
.DESCRIPTION
...
function EnableJsonSettings213 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '213'
        ScriptName = 'EnableJsonSettings213'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'EnableJsonSettings213.log'
...
$result = EnableJsonSettings213 -Destination $OutputPath
