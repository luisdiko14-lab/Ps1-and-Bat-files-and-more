<#
.SYNOPSIS
  Utility script 159 for lightweight workstation automation.
.DESCRIPTION
...
function ReadFolderInventory159 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '159'
        ScriptName = 'ReadFolderInventory159'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ReadFolderInventory159.log'
...
$result = ReadFolderInventory159 -Destination $OutputPath
