<#
.SYNOPSIS
  Utility script 100 for lightweight workstation automation.
.DESCRIPTION
...
function FormatServiceStatus100 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '100'
        ScriptName = 'FormatServiceStatus100'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'FormatServiceStatus100.log'
...
$result = FormatServiceStatus100 -Destination $OutputPath
