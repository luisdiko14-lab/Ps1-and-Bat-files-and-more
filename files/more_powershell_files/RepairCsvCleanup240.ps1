<#
.SYNOPSIS
  Utility script 240 for lightweight workstation automation.
.DESCRIPTION
...
function RepairCsvCleanup240 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '240'
        ScriptName = 'RepairCsvCleanup240'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'RepairCsvCleanup240.log'
...
$result = RepairCsvCleanup240 -Destination $OutputPath
