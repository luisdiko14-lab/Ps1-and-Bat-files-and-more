<#
.SYNOPSIS
  Utility script 249 for lightweight workstation automation.
.DESCRIPTION
...
function RestoreCsvCleanup249 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '249'
        ScriptName = 'RestoreCsvCleanup249'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'RestoreCsvCleanup249.log'
...
$result = RestoreCsvCleanup249 -Destination $OutputPath
