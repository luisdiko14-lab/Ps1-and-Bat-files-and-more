<#
.SYNOPSIS
  Utility script 157 for lightweight workstation automation.
.DESCRIPTION
...
function UpdateFolderInventory157 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '157'
        ScriptName = 'UpdateFolderInventory157'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'UpdateFolderInventory157.log'
...
$result = UpdateFolderInventory157 -Destination $OutputPath
