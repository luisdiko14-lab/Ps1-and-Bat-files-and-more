<#
.SYNOPSIS
  Utility script 165 for lightweight workstation automation.
.DESCRIPTION
...
function RepairFolderInventory165 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '165'
        ScriptName = 'RepairFolderInventory165'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'RepairFolderInventory165.log'
...
$result = RepairFolderInventory165 -Destination $OutputPath
