<#
.SYNOPSIS
  Utility script 095 for lightweight workstation automation.
.DESCRIPTION
...
function ImportServiceStatus095 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '095'
        ScriptName = 'ImportServiceStatus095'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ImportServiceStatus095.log'
...
$result = ImportServiceStatus095 -Destination $OutputPath
