<#
.SYNOPSIS
  Utility script 118 for lightweight workstation automation.
.DESCRIPTION
...
function WatchNetworkLatency118 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '118'
        ScriptName = 'WatchNetworkLatency118'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'WatchNetworkLatency118.log'
...
$result = WatchNetworkLatency118 -Destination $OutputPath
