<#
.SYNOPSIS
  Utility script 139 for lightweight workstation automation.
.DESCRIPTION
...
function DisableUptime139 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '139'
        ScriptName = 'DisableUptime139'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'DisableUptime139.log'
...
$result = DisableUptime139 -Destination $OutputPath
