<#
.SYNOPSIS
  Utility script 219 for lightweight workstation automation.
.DESCRIPTION
...
function ExportJsonSettings219 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '219'
        ScriptName = 'ExportJsonSettings219'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ExportJsonSettings219.log'
...
$result = ExportJsonSettings219 -Destination $OutputPath
