<#
.SYNOPSIS
  Utility script 170 for lightweight workstation automation.
.DESCRIPTION
...
function ImportFolderInventory170 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '170'
        ScriptName = 'ImportFolderInventory170'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ImportFolderInventory170.log'
...
$result = ImportFolderInventory170 -Destination $OutputPath
