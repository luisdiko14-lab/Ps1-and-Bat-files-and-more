<#
.SYNOPSIS
  Utility script 079 for lightweight workstation automation.
.DESCRIPTION
...
function InvokeServiceStatus079 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '079'
        ScriptName = 'InvokeServiceStatus079'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'InvokeServiceStatus079.log'
...
$result = InvokeServiceStatus079 -Destination $OutputPath
