<#
.SYNOPSIS
  Utility script 210 for lightweight workstation automation.
.DESCRIPTION
...
function NewJsonSettings210 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '210'
        ScriptName = 'NewJsonSettings210'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'NewJsonSettings210.log'
...
$result = NewJsonSettings210 -Destination $OutputPath
