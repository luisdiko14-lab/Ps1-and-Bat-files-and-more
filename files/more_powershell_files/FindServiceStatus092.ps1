<#
.SYNOPSIS
  Utility script 092 for lightweight workstation automation.
.DESCRIPTION
...
function FindServiceStatus092 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '092'
        ScriptName = 'FindServiceStatus092'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'FindServiceStatus092.log'
...
$result = FindServiceStatus092 -Destination $OutputPath
