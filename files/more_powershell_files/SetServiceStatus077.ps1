<#
.SYNOPSIS
  Utility script 077 for lightweight workstation automation.
.DESCRIPTION
...
function SetServiceStatus077 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '077'
        ScriptName = 'SetServiceStatus077'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'SetServiceStatus077.log'
...
$result = SetServiceStatus077 -Destination $OutputPath
