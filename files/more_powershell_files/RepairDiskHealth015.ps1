<#
.SYNOPSIS
  Utility script 015 for lightweight workstation automation.
.DESCRIPTION
...
function RepairDiskHealth015 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '015'
        ScriptName = 'RepairDiskHealth015'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'RepairDiskHealth015.log'
...
$result = RepairDiskHealth015 -Destination $OutputPath
