<#
.SYNOPSIS
  Utility script 131 for lightweight workstation automation.
.DESCRIPTION
...
function ConvertUptime131 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '131'
        ScriptName = 'ConvertUptime131'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ConvertUptime131.log'
...
$result = ConvertUptime131 -Destination $OutputPath
