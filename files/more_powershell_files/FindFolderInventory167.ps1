<#
.SYNOPSIS
  Utility script 167 for lightweight workstation automation.
.DESCRIPTION
...
function FindFolderInventory167 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '167'
        ScriptName = 'FindFolderInventory167'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'FindFolderInventory167.log'
...
$result = FindFolderInventory167 -Destination $OutputPath
