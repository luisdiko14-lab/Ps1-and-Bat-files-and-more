<#
.SYNOPSIS
  Utility script 083 for lightweight workstation automation.
.DESCRIPTION
...
function WriteServiceStatus083 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '083'
        ScriptName = 'WriteServiceStatus083'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'WriteServiceStatus083.log'
...
$result = WriteServiceStatus083 -Destination $OutputPath
