<#
.SYNOPSIS
  Utility script 022 for lightweight workstation automation.
.DESCRIPTION
...
function OptimizeDiskHealth022 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '022'
        ScriptName = 'OptimizeDiskHealth022'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'OptimizeDiskHealth022.log'
...
$result = OptimizeDiskHealth022 -Destination $OutputPath
