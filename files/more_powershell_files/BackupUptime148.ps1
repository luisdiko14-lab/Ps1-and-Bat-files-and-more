<#
.SYNOPSIS
  Utility script 148 for lightweight workstation automation.
.DESCRIPTION
...
function BackupUptime148 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '148'
        ScriptName = 'BackupUptime148'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'BackupUptime148.log'
...
$result = BackupUptime148 -Destination $OutputPath
