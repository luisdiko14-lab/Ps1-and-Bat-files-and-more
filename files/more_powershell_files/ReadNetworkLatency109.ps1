<#
.SYNOPSIS
  Utility script 109 for lightweight workstation automation.
.DESCRIPTION
...
function ReadNetworkLatency109 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '109'
        ScriptName = 'ReadNetworkLatency109'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ReadNetworkLatency109.log'
...
$result = ReadNetworkLatency109 -Destination $OutputPath
