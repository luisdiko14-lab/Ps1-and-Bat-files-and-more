<#
.SYNOPSIS
  Utility script 246 for lightweight workstation automation.
.DESCRIPTION
...
function CompareCsvCleanup246 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '246'
        ScriptName = 'CompareCsvCleanup246'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'CompareCsvCleanup246.log'
...
$result = CompareCsvCleanup246 -Destination $OutputPath
