<#
.SYNOPSIS
  Utility script 038 for lightweight workstation automation.
.DESCRIPTION
...
function EnableEventLogSummary038 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '038'
        ScriptName = 'EnableEventLogSummary038'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'EnableEventLogSummary038.log'
...
$result = EnableEventLogSummary038 -Destination $OutputPath
