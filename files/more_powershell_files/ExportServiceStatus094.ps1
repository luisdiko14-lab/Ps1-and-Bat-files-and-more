<#
.SYNOPSIS
  Utility script 094 for lightweight workstation automation.
.DESCRIPTION
...
function ExportServiceStatus094 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '094'
        ScriptName = 'ExportServiceStatus094'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ExportServiceStatus094.log'
...
$result = ExportServiceStatus094 -Destination $OutputPath
