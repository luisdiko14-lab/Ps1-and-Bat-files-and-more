<#
.SYNOPSIS
  Utility script 141 for lightweight workstation automation.
.DESCRIPTION
...
function ClearUptime141 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '141'
        ScriptName = 'ClearUptime141'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ClearUptime141.log'
...
$result = ClearUptime141 -Destination $OutputPath
