<#
.SYNOPSIS
  Utility script 088 for lightweight workstation automation.
.DESCRIPTION
...
function EnableServiceStatus088 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '088'
        ScriptName = 'EnableServiceStatus088'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'EnableServiceStatus088.log'
...
$result = EnableServiceStatus088 -Destination $OutputPath
