<#
.SYNOPSIS
  Utility script 250 for lightweight workstation automation.
.DESCRIPTION
...
function FormatCsvCleanup250 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '250'
        ScriptName = 'FormatCsvCleanup250'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'FormatCsvCleanup250.log'
...
$result = FormatCsvCleanup250 -Destination $OutputPath
