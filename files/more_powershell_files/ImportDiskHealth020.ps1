<#
.SYNOPSIS
  Utility script 020 for lightweight workstation automation.
.DESCRIPTION
...
function ImportDiskHealth020 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '020'
        ScriptName = 'ImportDiskHealth020'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ImportDiskHealth020.log'
...
$result = ImportDiskHealth020 -Destination $OutputPath
