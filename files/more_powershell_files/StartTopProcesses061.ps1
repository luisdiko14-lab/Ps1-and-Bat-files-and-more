<#
.SYNOPSIS
  Utility script 061 for lightweight workstation automation.
.DESCRIPTION
...
function StartTopProcesses061 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '061'
        ScriptName = 'StartTopProcesses061'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'StartTopProcesses061.log'
...
$result = StartTopProcesses061 -Destination $OutputPath
