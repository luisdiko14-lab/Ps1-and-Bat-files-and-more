<#
.SYNOPSIS
  Utility script 043 for lightweight workstation automation.
.DESCRIPTION
...
function WatchEventLogSummary043 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '043'
        ScriptName = 'WatchEventLogSummary043'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'WatchEventLogSummary043.log'
...
$result = WatchEventLogSummary043 -Destination $OutputPath
