<#
.SYNOPSIS
  Utility script 135 for lightweight workstation automation.
.DESCRIPTION
...
function NewUptime135 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '135'
        ScriptName = 'NewUptime135'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'NewUptime135.log'
...
$result = NewUptime135 -Destination $OutputPath
