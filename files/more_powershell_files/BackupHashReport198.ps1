<#
.SYNOPSIS
  Utility script 198 for lightweight workstation automation.
.DESCRIPTION
...
function BackupHashReport198 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '198'
        ScriptName = 'BackupHashReport198'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'BackupHashReport198.log'
...
$result = BackupHashReport198 -Destination $OutputPath
