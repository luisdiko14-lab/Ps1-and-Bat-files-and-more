<#
.SYNOPSIS
  Utility script 082 for lightweight workstation automation.
.DESCRIPTION
...
function UpdateServiceStatus082 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '082'
        ScriptName = 'UpdateServiceStatus082'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'UpdateServiceStatus082.log'
...
$result = UpdateServiceStatus082 -Destination $OutputPath
