<#
.SYNOPSIS
  Utility script 097 for lightweight workstation automation.
.DESCRIPTION
...
function OptimizeServiceStatus097 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '097'
        ScriptName = 'OptimizeServiceStatus097'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'OptimizeServiceStatus097.log'
...
$result = OptimizeServiceStatus097 -Destination $OutputPath
