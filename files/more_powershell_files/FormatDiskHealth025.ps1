<#
.SYNOPSIS
  Utility script 025 for lightweight workstation automation.
.DESCRIPTION
...
function FormatDiskHealth025 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '025'
        ScriptName = 'FormatDiskHealth025'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'FormatDiskHealth025.log'
...
$result = FormatDiskHealth025 -Destination $OutputPath
