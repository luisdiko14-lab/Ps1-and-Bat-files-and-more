<#
.SYNOPSIS
  Utility script 212 for lightweight workstation automation.
.DESCRIPTION
...
function StopJsonSettings212 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '212'
        ScriptName = 'StopJsonSettings212'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'StopJsonSettings212.log'
...
$result = StopJsonSettings212 -Destination $OutputPath
