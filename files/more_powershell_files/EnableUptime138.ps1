<#
.SYNOPSIS
  Utility script 138 for lightweight workstation automation.
.DESCRIPTION
...
function EnableUptime138 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '138'
        ScriptName = 'EnableUptime138'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'EnableUptime138.log'
...
$result = EnableUptime138 -Destination $OutputPath
