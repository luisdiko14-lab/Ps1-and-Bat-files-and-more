<#
.SYNOPSIS
  Utility script 172 for lightweight workstation automation.
.DESCRIPTION
...
function OptimizeFolderInventory172 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '172'
        ScriptName = 'OptimizeFolderInventory172'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'OptimizeFolderInventory172.log'
...
$result = OptimizeFolderInventory172 -Destination $OutputPath
