<#
.SYNOPSIS
  Utility script 050 for lightweight workstation automation.
.DESCRIPTION
...
function FormatEventLogSummary050 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '050'
        ScriptName = 'FormatEventLogSummary050'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'FormatEventLogSummary050.log'
...
$result = FormatEventLogSummary050 -Destination $OutputPath
