<#
.SYNOPSIS
  Utility script 067 for lightweight workstation automation.
.DESCRIPTION
...
function FindTopProcesses067 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '067'
        ScriptName = 'FindTopProcesses067'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'FindTopProcesses067.log'
...
$result = FindTopProcesses067 -Destination $OutputPath
