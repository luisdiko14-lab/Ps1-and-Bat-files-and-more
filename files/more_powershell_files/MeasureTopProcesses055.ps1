<#
.SYNOPSIS
  Utility script 055 for lightweight workstation automation.
.DESCRIPTION
...
function MeasureTopProcesses055 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '055'
        ScriptName = 'MeasureTopProcesses055'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'MeasureTopProcesses055.log'
...
$result = MeasureTopProcesses055 -Destination $OutputPath
