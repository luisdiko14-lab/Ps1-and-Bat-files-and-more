<#
.SYNOPSIS
  Utility script 099 for lightweight workstation automation.
.DESCRIPTION
...
function RestoreServiceStatus099 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '099'
        ScriptName = 'RestoreServiceStatus099'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'RestoreServiceStatus099.log'
...
$result = RestoreServiceStatus099 -Destination $OutputPath
