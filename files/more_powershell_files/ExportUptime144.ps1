<#
.SYNOPSIS
  Utility script 144 for lightweight workstation automation.
.DESCRIPTION
...
function ExportUptime144 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '144'
        ScriptName = 'ExportUptime144'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ExportUptime144.log'
...
$result = ExportUptime144 -Destination $OutputPath
