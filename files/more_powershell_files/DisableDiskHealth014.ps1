<#
.SYNOPSIS
  Utility script 014 for lightweight workstation automation.
.DESCRIPTION
...
function DisableDiskHealth014 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '014'
        ScriptName = 'DisableDiskHealth014'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'DisableDiskHealth014.log'
...
$result = DisableDiskHealth014 -Destination $OutputPath
