<#
.SYNOPSIS
  Utility script 136 for lightweight workstation automation.
.DESCRIPTION
...
function StartUptime136 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '136'
        ScriptName = 'StartUptime136'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'StartUptime136.log'
...
$result = StartUptime136 -Destination $OutputPath
