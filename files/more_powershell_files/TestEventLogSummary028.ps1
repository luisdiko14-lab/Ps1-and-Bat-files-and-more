<#
.SYNOPSIS
  Utility script 028 for lightweight workstation automation.
.DESCRIPTION
...
function TestEventLogSummary028 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '028'
        ScriptName = 'TestEventLogSummary028'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'TestEventLogSummary028.log'
...
$result = TestEventLogSummary028 -Destination $OutputPath
