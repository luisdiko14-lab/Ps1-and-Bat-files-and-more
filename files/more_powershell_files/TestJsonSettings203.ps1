<#
.SYNOPSIS
  Utility script 203 for lightweight workstation automation.
.DESCRIPTION
...
function TestJsonSettings203 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '203'
        ScriptName = 'TestJsonSettings203'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'TestJsonSettings203.log'
...
$result = TestJsonSettings203 -Destination $OutputPath
