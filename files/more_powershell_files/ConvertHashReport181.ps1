<#
.SYNOPSIS
  Utility script 181 for lightweight workstation automation.
.DESCRIPTION
...
function ConvertHashReport181 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '181'
        ScriptName = 'ConvertHashReport181'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ConvertHashReport181.log'
...
$result = ConvertHashReport181 -Destination $OutputPath
