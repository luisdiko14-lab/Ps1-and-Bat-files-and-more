<#
.SYNOPSIS
  Utility script 037 for lightweight workstation automation.
.DESCRIPTION
...
function StopEventLogSummary037 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '037'
        ScriptName = 'StopEventLogSummary037'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'StopEventLogSummary037.log'
...
$result = StopEventLogSummary037 -Destination $OutputPath
