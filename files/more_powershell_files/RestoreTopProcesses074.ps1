<#
.SYNOPSIS
  Utility script 074 for lightweight workstation automation.
.DESCRIPTION
...
function RestoreTopProcesses074 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '074'
        ScriptName = 'RestoreTopProcesses074'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'RestoreTopProcesses074.log'
...
$result = RestoreTopProcesses074 -Destination $OutputPath
