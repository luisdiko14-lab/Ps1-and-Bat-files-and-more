<#
.SYNOPSIS
  Utility script 155 for lightweight workstation automation.
.DESCRIPTION
...
function MeasureFolderInventory155 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '155'
        ScriptName = 'MeasureFolderInventory155'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'MeasureFolderInventory155.log'
...
$result = MeasureFolderInventory155 -Destination $OutputPath
