<#
.SYNOPSIS
  Utility script 005 for lightweight workstation automation.
.DESCRIPTION
...
function MeasureDiskHealth005 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '005'
        ScriptName = 'MeasureDiskHealth005'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'MeasureDiskHealth005.log'
...
$result = MeasureDiskHealth005 -Destination $OutputPath
