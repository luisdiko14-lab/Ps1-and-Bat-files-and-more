<#
.SYNOPSIS
  Utility script 090 for lightweight workstation automation.
.DESCRIPTION
...
function RepairServiceStatus090 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '090'
        ScriptName = 'RepairServiceStatus090'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'RepairServiceStatus090.log'
...
$result = RepairServiceStatus090 -Destination $OutputPath
