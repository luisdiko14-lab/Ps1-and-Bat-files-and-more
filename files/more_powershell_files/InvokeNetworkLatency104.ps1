<#
.SYNOPSIS
  Utility script 104 for lightweight workstation automation.
.DESCRIPTION
...
function InvokeNetworkLatency104 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '104'
        ScriptName = 'InvokeNetworkLatency104'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'InvokeNetworkLatency104.log'
...
$result = InvokeNetworkLatency104 -Destination $OutputPath
