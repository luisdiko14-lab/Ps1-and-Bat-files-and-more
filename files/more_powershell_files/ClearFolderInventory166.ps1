<#
.SYNOPSIS
  Utility script 166 for lightweight workstation automation.
.DESCRIPTION
...
function ClearFolderInventory166 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '166'
        ScriptName = 'ClearFolderInventory166'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ClearFolderInventory166.log'
...
$result = ClearFolderInventory166 -Destination $OutputPath
