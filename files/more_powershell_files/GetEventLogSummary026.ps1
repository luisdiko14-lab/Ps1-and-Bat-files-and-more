<#
.SYNOPSIS
  Utility script 026 for lightweight workstation automation.
.DESCRIPTION
...
function GetEventLogSummary026 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '026'
        ScriptName = 'GetEventLogSummary026'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'GetEventLogSummary026.log'
...
$result = GetEventLogSummary026 -Destination $OutputPath
