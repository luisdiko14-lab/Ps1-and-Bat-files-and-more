<#
.SYNOPSIS
  Utility script 238 for lightweight workstation automation.
.DESCRIPTION
...
function EnableCsvCleanup238 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '238'
        ScriptName = 'EnableCsvCleanup238'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'EnableCsvCleanup238.log'
...
$result = EnableCsvCleanup238 -Destination $OutputPath
