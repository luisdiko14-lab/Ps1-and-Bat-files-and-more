<#
.SYNOPSIS
  Utility script 112 for lightweight workstation automation.
.DESCRIPTION
...
function StopNetworkLatency112 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '112'
        ScriptName = 'StopNetworkLatency112'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'StopNetworkLatency112.log'
...
$result = StopNetworkLatency112 -Destination $OutputPath
