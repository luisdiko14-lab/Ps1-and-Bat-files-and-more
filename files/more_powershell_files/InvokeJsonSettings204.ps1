<#
.SYNOPSIS
  Utility script 204 for lightweight workstation automation.
.DESCRIPTION
...
function InvokeJsonSettings204 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '204'
        ScriptName = 'InvokeJsonSettings204'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'InvokeJsonSettings204.log'
...
$result = InvokeJsonSettings204 -Destination $OutputPath
