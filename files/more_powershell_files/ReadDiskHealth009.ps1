<#
.SYNOPSIS
  Utility script 009 for lightweight workstation automation.
.DESCRIPTION
...
function ReadDiskHealth009 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '009'
        ScriptName = 'ReadDiskHealth009'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ReadDiskHealth009.log'
...
$result = ReadDiskHealth009 -Destination $OutputPath
