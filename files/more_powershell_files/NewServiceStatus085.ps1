<#
.SYNOPSIS
  Utility script 085 for lightweight workstation automation.
.DESCRIPTION
...
function NewServiceStatus085 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '085'
        ScriptName = 'NewServiceStatus085'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'NewServiceStatus085.log'
...
$result = NewServiceStatus085 -Destination $OutputPath
