<#
.SYNOPSIS
  Utility script 225 for lightweight workstation automation.
.DESCRIPTION
...
function FormatJsonSettings225 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '225'
        ScriptName = 'FormatJsonSettings225'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'FormatJsonSettings225.log'
...
$result = FormatJsonSettings225 -Destination $OutputPath
