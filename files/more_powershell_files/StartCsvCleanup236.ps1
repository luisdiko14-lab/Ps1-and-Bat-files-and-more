<#
.SYNOPSIS
  Utility script 236 for lightweight workstation automation.
.DESCRIPTION
...
function StartCsvCleanup236 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '236'
        ScriptName = 'StartCsvCleanup236'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'StartCsvCleanup236.log'
...
$result = StartCsvCleanup236 -Destination $OutputPath
