<#
.SYNOPSIS
  Utility script 222 for lightweight workstation automation.
.DESCRIPTION
...
function OptimizeJsonSettings222 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '222'
        ScriptName = 'OptimizeJsonSettings222'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'OptimizeJsonSettings222.log'
...
$result = OptimizeJsonSettings222 -Destination $OutputPath
