<#
.SYNOPSIS
  Utility script 232 for lightweight workstation automation.
.DESCRIPTION
...
function UpdateCsvCleanup232 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '232'
        ScriptName = 'UpdateCsvCleanup232'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'UpdateCsvCleanup232.log'
...
$result = UpdateCsvCleanup232 -Destination $OutputPath
