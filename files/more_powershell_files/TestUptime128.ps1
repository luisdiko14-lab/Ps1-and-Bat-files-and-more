<#
.SYNOPSIS
  Utility script 128 for lightweight workstation automation.
.DESCRIPTION
...
function TestUptime128 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '128'
        ScriptName = 'TestUptime128'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'TestUptime128.log'
...
$result = TestUptime128 -Destination $OutputPath
