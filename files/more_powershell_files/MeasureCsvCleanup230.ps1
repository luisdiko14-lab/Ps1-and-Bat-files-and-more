<#
.SYNOPSIS
  Utility script 230 for lightweight workstation automation.
.DESCRIPTION
...
function MeasureCsvCleanup230 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '230'
        ScriptName = 'MeasureCsvCleanup230'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'MeasureCsvCleanup230.log'
...
$result = MeasureCsvCleanup230 -Destination $OutputPath
