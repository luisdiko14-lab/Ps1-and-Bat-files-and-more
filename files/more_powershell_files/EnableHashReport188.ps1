<#
.SYNOPSIS
  Utility script 188 for lightweight workstation automation.
.DESCRIPTION
...
function EnableHashReport188 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '188'
        ScriptName = 'EnableHashReport188'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'EnableHashReport188.log'
...
$result = EnableHashReport188 -Destination $OutputPath
