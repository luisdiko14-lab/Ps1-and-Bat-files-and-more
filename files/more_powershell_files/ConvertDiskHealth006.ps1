<#
.SYNOPSIS
  Utility script 006 for lightweight workstation automation.
.DESCRIPTION
...
function ConvertDiskHealth006 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '006'
        ScriptName = 'ConvertDiskHealth006'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ConvertDiskHealth006.log'
...
$result = ConvertDiskHealth006 -Destination $OutputPath
