<#
.SYNOPSIS
  Utility script 145 for lightweight workstation automation.
.DESCRIPTION
...
function ImportUptime145 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '145'
        ScriptName = 'ImportUptime145'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ImportUptime145.log'
...
$result = ImportUptime145 -Destination $OutputPath
