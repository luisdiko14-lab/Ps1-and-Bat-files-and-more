<#
.SYNOPSIS
  Utility script 029 for lightweight workstation automation.
.DESCRIPTION
...
function InvokeEventLogSummary029 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '029'
        ScriptName = 'InvokeEventLogSummary029'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'InvokeEventLogSummary029.log'
...
$result = InvokeEventLogSummary029 -Destination $OutputPath
