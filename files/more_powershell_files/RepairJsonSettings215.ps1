<#
.SYNOPSIS
  Utility script 215 for lightweight workstation automation.
.DESCRIPTION
...
function RepairJsonSettings215 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '215'
        ScriptName = 'RepairJsonSettings215'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'RepairJsonSettings215.log'
...
$result = RepairJsonSettings215 -Destination $OutputPath
