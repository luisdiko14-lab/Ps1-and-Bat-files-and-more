<#
.SYNOPSIS
  Utility script 176 for lightweight workstation automation.
.DESCRIPTION
...
function GetHashReport176 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '176'
        ScriptName = 'GetHashReport176'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'GetHashReport176.log'
...
$result = GetHashReport176 -Destination $OutputPath
