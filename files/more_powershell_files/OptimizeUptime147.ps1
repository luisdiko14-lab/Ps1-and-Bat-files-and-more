<#
.SYNOPSIS
  Utility script 147 for lightweight workstation automation.
.DESCRIPTION
...
function OptimizeUptime147 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '147'
        ScriptName = 'OptimizeUptime147'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'OptimizeUptime147.log'
...
$result = OptimizeUptime147 -Destination $OutputPath
