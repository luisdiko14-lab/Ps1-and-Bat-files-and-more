<#
.SYNOPSIS
  Utility script 133 for lightweight workstation automation.
.DESCRIPTION
...
function WriteUptime133 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '133'
        ScriptName = 'WriteUptime133'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'WriteUptime133.log'
...
$result = WriteUptime133 -Destination $OutputPath
