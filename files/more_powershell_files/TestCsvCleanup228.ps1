<#
.SYNOPSIS
  Utility script 228 for lightweight workstation automation.
.DESCRIPTION
...
function TestCsvCleanup228 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '228'
        ScriptName = 'TestCsvCleanup228'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'TestCsvCleanup228.log'
...
$result = TestCsvCleanup228 -Destination $OutputPath
