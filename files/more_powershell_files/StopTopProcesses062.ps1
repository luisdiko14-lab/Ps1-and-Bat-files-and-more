<#
.SYNOPSIS
  Utility script 062 for lightweight workstation automation.
.DESCRIPTION
...
function StopTopProcesses062 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '062'
        ScriptName = 'StopTopProcesses062'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'StopTopProcesses062.log'
...
$result = StopTopProcesses062 -Destination $OutputPath
