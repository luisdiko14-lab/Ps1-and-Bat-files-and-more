<#
.SYNOPSIS
  Utility script 031 for lightweight workstation automation.
.DESCRIPTION
...
function ConvertEventLogSummary031 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '031'
        ScriptName = 'ConvertEventLogSummary031'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ConvertEventLogSummary031.log'
...
$result = ConvertEventLogSummary031 -Destination $OutputPath
