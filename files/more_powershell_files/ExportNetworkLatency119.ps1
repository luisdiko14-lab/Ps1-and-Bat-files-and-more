<#
.SYNOPSIS
  Utility script 119 for lightweight workstation automation.
.DESCRIPTION
...
function ExportNetworkLatency119 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '119'
        ScriptName = 'ExportNetworkLatency119'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ExportNetworkLatency119.log'
...
$result = ExportNetworkLatency119 -Destination $OutputPath
