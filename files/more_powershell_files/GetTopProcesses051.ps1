<#
.SYNOPSIS
  Utility script 051 for lightweight workstation automation.
.DESCRIPTION
...
function GetTopProcesses051 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '051'
        ScriptName = 'GetTopProcesses051'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'GetTopProcesses051.log'
...
$result = GetTopProcesses051 -Destination $OutputPath
