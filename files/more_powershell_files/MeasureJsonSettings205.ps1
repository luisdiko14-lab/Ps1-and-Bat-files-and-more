<#
.SYNOPSIS
  Utility script 205 for lightweight workstation automation.
.DESCRIPTION
...
function MeasureJsonSettings205 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '205'
        ScriptName = 'MeasureJsonSettings205'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'MeasureJsonSettings205.log'
...
$result = MeasureJsonSettings205 -Destination $OutputPath
