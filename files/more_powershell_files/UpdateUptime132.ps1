<#
.SYNOPSIS
  Utility script 132 for lightweight workstation automation.
.DESCRIPTION
...
function UpdateUptime132 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '132'
        ScriptName = 'UpdateUptime132'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'UpdateUptime132.log'
...
$result = UpdateUptime132 -Destination $OutputPath
