<#
.SYNOPSIS
  Utility script 084 for lightweight workstation automation.
.DESCRIPTION
...
function ReadServiceStatus084 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '084'
        ScriptName = 'ReadServiceStatus084'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ReadServiceStatus084.log'
...
$result = ReadServiceStatus084 -Destination $OutputPath
