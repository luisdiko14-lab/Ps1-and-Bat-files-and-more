<#
.SYNOPSIS
  Utility script 231 for lightweight workstation automation.
.DESCRIPTION
...
function ConvertCsvCleanup231 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '231'
        ScriptName = 'ConvertCsvCleanup231'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ConvertCsvCleanup231.log'
...
$result = ConvertCsvCleanup231 -Destination $OutputPath
