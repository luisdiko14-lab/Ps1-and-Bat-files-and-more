<#
.SYNOPSIS
  Utility script 182 for lightweight workstation automation.
.DESCRIPTION
...
function UpdateHashReport182 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '182'
        ScriptName = 'UpdateHashReport182'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'UpdateHashReport182.log'
...
$result = UpdateHashReport182 -Destination $OutputPath
