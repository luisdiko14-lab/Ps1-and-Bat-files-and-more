<#
.SYNOPSIS
  Utility script 034 for lightweight workstation automation.
.DESCRIPTION
...
function ReadEventLogSummary034 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '034'
        ScriptName = 'ReadEventLogSummary034'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ReadEventLogSummary034.log'
...
$result = ReadEventLogSummary034 -Destination $OutputPath
