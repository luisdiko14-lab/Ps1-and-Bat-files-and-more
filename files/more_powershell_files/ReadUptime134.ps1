<#
.SYNOPSIS
  Utility script 134 for lightweight workstation automation.
.DESCRIPTION
...
function ReadUptime134 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '134'
        ScriptName = 'ReadUptime134'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ReadUptime134.log'
...
$result = ReadUptime134 -Destination $OutputPath
