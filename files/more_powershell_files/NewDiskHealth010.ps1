<#
.SYNOPSIS
  Utility script 010 for lightweight workstation automation.
.DESCRIPTION
...
function NewDiskHealth010 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '010'
        ScriptName = 'NewDiskHealth010'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'NewDiskHealth010.log'
...
$result = NewDiskHealth010 -Destination $OutputPath
