<#
.SYNOPSIS
  Utility script 190 for lightweight workstation automation.
.DESCRIPTION
...
function RepairHashReport190 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '190'
        ScriptName = 'RepairHashReport190'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'RepairHashReport190.log'
...
$result = RepairHashReport190 -Destination $OutputPath
