<#
.SYNOPSIS
  Utility script 153 for lightweight workstation automation.
.DESCRIPTION
...
function TestFolderInventory153 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '153'
        ScriptName = 'TestFolderInventory153'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'TestFolderInventory153.log'
...
$result = TestFolderInventory153 -Destination $OutputPath
