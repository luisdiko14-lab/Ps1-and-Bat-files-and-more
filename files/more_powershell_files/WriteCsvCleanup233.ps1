<#
.SYNOPSIS
  Utility script 233 for lightweight workstation automation.
.DESCRIPTION
...
function WriteCsvCleanup233 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '233'
        ScriptName = 'WriteCsvCleanup233'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'WriteCsvCleanup233.log'
...
$result = WriteCsvCleanup233 -Destination $OutputPath
