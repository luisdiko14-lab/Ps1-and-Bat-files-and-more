<#
.SYNOPSIS
  Utility script 042 for lightweight workstation automation.
.DESCRIPTION
...
function FindEventLogSummary042 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '042'
        ScriptName = 'FindEventLogSummary042'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'FindEventLogSummary042.log'
...
$result = FindEventLogSummary042 -Destination $OutputPath
