<#
.SYNOPSIS
  Utility script 023 for lightweight workstation automation.
.DESCRIPTION
...
function BackupDiskHealth023 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '023'
        ScriptName = 'BackupDiskHealth023'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'BackupDiskHealth023.log'
...
$result = BackupDiskHealth023 -Destination $OutputPath
