<#
.SYNOPSIS
  Utility script 108 for lightweight workstation automation.
.DESCRIPTION
...
function WriteNetworkLatency108 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '108'
        ScriptName = 'WriteNetworkLatency108'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'WriteNetworkLatency108.log'
...
$result = WriteNetworkLatency108 -Destination $OutputPath
