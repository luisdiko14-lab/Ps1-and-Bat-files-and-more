<#
.SYNOPSIS
  Utility script 201 for lightweight workstation automation.
.DESCRIPTION
...
function GetJsonSettings201 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '201'
        ScriptName = 'GetJsonSettings201'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'GetJsonSettings201.log'
...
$result = GetJsonSettings201 -Destination $OutputPath
