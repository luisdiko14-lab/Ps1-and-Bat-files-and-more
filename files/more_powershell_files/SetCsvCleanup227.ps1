<#
.SYNOPSIS
  Utility script 227 for lightweight workstation automation.
.DESCRIPTION
...
function SetCsvCleanup227 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '227'
        ScriptName = 'SetCsvCleanup227'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'SetCsvCleanup227.log'
...
$result = SetCsvCleanup227 -Destination $OutputPath
