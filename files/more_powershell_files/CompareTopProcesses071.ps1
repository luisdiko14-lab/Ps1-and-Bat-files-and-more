<#
.SYNOPSIS
  Utility script 071 for lightweight workstation automation.
.DESCRIPTION
...
function CompareTopProcesses071 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '071'
        ScriptName = 'CompareTopProcesses071'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'CompareTopProcesses071.log'
...
$result = CompareTopProcesses071 -Destination $OutputPath
