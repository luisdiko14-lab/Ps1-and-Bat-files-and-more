<#
.SYNOPSIS
  Utility script 064 for lightweight workstation automation.
.DESCRIPTION
...
function DisableTopProcesses064 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '064'
        ScriptName = 'DisableTopProcesses064'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'DisableTopProcesses064.log'
...
$result = DisableTopProcesses064 -Destination $OutputPath
