<#
.SYNOPSIS
  Utility script 072 for lightweight workstation automation.
.DESCRIPTION
...
function OptimizeTopProcesses072 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '072'
        ScriptName = 'OptimizeTopProcesses072'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'OptimizeTopProcesses072.log'
...
$result = OptimizeTopProcesses072 -Destination $OutputPath
