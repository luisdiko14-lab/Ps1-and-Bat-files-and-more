<#
.SYNOPSIS
  Utility script 121 for lightweight workstation automation.
.DESCRIPTION
...
function CompareNetworkLatency121 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '121'
        ScriptName = 'CompareNetworkLatency121'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'CompareNetworkLatency121.log'
...
$result = CompareNetworkLatency121 -Destination $OutputPath
