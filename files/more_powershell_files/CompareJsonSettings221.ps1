<#
.SYNOPSIS
  Utility script 221 for lightweight workstation automation.
.DESCRIPTION
...
function CompareJsonSettings221 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '221'
        ScriptName = 'CompareJsonSettings221'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'CompareJsonSettings221.log'
...
$result = CompareJsonSettings221 -Destination $OutputPath
