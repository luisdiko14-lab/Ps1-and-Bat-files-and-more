<#
.SYNOPSIS
  Utility script 035 for lightweight workstation automation.
.DESCRIPTION
...
function NewEventLogSummary035 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '035'
        ScriptName = 'NewEventLogSummary035'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'NewEventLogSummary035.log'
...
$result = NewEventLogSummary035 -Destination $OutputPath
