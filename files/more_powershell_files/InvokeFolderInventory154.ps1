<#
.SYNOPSIS
  Utility script 154 for lightweight workstation automation.
.DESCRIPTION
...
function InvokeFolderInventory154 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '154'
        ScriptName = 'InvokeFolderInventory154'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'InvokeFolderInventory154.log'
...
$result = InvokeFolderInventory154 -Destination $OutputPath
