<#
.SYNOPSIS
  Utility script 245 for lightweight workstation automation.
.DESCRIPTION
...
function ImportCsvCleanup245 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '245'
        ScriptName = 'ImportCsvCleanup245'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ImportCsvCleanup245.log'
...
$result = ImportCsvCleanup245 -Destination $OutputPath
