<#
.SYNOPSIS
  Utility script 237 for lightweight workstation automation.
.DESCRIPTION
...
function StopCsvCleanup237 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '237'
        ScriptName = 'StopCsvCleanup237'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'StopCsvCleanup237.log'
...
$result = StopCsvCleanup237 -Destination $OutputPath
