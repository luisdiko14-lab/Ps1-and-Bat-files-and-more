<#
.SYNOPSIS
  Utility script 242 for lightweight workstation automation.
.DESCRIPTION
...
function FindCsvCleanup242 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '242'
        ScriptName = 'FindCsvCleanup242'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'FindCsvCleanup242.log'
...
$result = FindCsvCleanup242 -Destination $OutputPath
