<#
.SYNOPSIS
  Utility script 058 for lightweight workstation automation.
.DESCRIPTION
...
function WriteTopProcesses058 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '058'
        ScriptName = 'WriteTopProcesses058'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'WriteTopProcesses058.log'
...
$result = WriteTopProcesses058 -Destination $OutputPath
