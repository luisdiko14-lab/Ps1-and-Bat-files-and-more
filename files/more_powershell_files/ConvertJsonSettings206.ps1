<#
.SYNOPSIS
  Utility script 206 for lightweight workstation automation.
.DESCRIPTION
...
function ConvertJsonSettings206 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '206'
        ScriptName = 'ConvertJsonSettings206'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ConvertJsonSettings206.log'
...
$result = ConvertJsonSettings206 -Destination $OutputPath
