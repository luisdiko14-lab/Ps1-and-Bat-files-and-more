<#
.SYNOPSIS
  Utility script 161 for lightweight workstation automation.
.DESCRIPTION
...
function StartFolderInventory161 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '161'
        ScriptName = 'StartFolderInventory161'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'StartFolderInventory161.log'
...
$result = StartFolderInventory161 -Destination $OutputPath
