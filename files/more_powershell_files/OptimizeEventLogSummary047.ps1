<#
.SYNOPSIS
  Utility script 047 for lightweight workstation automation.
.DESCRIPTION
...
function OptimizeEventLogSummary047 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '047'
        ScriptName = 'OptimizeEventLogSummary047'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'OptimizeEventLogSummary047.log'
...
$result = OptimizeEventLogSummary047 -Destination $OutputPath
