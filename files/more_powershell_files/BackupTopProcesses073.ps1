<#
.SYNOPSIS
  Utility script 073 for lightweight workstation automation.
.DESCRIPTION
...
function BackupTopProcesses073 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '073'
        ScriptName = 'BackupTopProcesses073'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'BackupTopProcesses073.log'
...
$result = BackupTopProcesses073 -Destination $OutputPath
