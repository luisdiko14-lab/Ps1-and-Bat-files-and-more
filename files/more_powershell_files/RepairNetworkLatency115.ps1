<#
.SYNOPSIS
  Utility script 115 for lightweight workstation automation.
.DESCRIPTION
...
function RepairNetworkLatency115 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '115'
        ScriptName = 'RepairNetworkLatency115'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'RepairNetworkLatency115.log'
...
$result = RepairNetworkLatency115 -Destination $OutputPath
