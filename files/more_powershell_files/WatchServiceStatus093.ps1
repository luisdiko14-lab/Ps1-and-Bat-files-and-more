<#
.SYNOPSIS
  Utility script 093 for lightweight workstation automation.
.DESCRIPTION
...
function WatchServiceStatus093 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '093'
        ScriptName = 'WatchServiceStatus093'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'WatchServiceStatus093.log'
...
$result = WatchServiceStatus093 -Destination $OutputPath
