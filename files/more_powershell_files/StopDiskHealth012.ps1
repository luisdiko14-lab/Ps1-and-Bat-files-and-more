<#
.SYNOPSIS
  Utility script 012 for lightweight workstation automation.
.DESCRIPTION
...
function StopDiskHealth012 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '012'
        ScriptName = 'StopDiskHealth012'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'StopDiskHealth012.log'
...
$result = StopDiskHealth012 -Destination $OutputPath
