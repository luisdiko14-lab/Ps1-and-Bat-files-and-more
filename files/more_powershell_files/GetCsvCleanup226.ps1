<#
.SYNOPSIS
  Utility script 226 for lightweight workstation automation.
.DESCRIPTION
...
function GetCsvCleanup226 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '226'
        ScriptName = 'GetCsvCleanup226'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'GetCsvCleanup226.log'
...
$result = GetCsvCleanup226 -Destination $OutputPath
