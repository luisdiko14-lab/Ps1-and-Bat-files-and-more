<#
.SYNOPSIS
  Utility script 126 for lightweight workstation automation.
.DESCRIPTION
...
function GetUptime126 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '126'
        ScriptName = 'GetUptime126'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'GetUptime126.log'
...
$result = GetUptime126 -Destination $OutputPath
