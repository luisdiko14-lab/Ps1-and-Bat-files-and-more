<#
.SYNOPSIS
  Utility script 039 for lightweight workstation automation.
.DESCRIPTION
...
function DisableEventLogSummary039 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '039'
        ScriptName = 'DisableEventLogSummary039'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'DisableEventLogSummary039.log'
...
$result = DisableEventLogSummary039 -Destination $OutputPath
