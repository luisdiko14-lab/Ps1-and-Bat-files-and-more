<#
.SYNOPSIS
  Utility script 173 for lightweight workstation automation.
.DESCRIPTION
...
function BackupFolderInventory173 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '173'
        ScriptName = 'BackupFolderInventory173'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'BackupFolderInventory173.log'
...
$result = BackupFolderInventory173 -Destination $OutputPath
