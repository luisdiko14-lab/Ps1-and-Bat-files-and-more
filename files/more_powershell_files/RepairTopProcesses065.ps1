<#
.SYNOPSIS
  Utility script 065 for lightweight workstation automation.
.DESCRIPTION
...
function RepairTopProcesses065 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '065'
        ScriptName = 'RepairTopProcesses065'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'RepairTopProcesses065.log'
...
$result = RepairTopProcesses065 -Destination $OutputPath
