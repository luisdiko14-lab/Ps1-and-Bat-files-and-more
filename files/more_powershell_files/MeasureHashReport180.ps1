<#
.SYNOPSIS
  Utility script 180 for lightweight workstation automation.
.DESCRIPTION
...
function MeasureHashReport180 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '180'
        ScriptName = 'MeasureHashReport180'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'MeasureHashReport180.log'
...
$result = MeasureHashReport180 -Destination $OutputPath
