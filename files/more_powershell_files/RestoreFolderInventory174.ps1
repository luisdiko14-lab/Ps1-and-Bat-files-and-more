<#
.SYNOPSIS
  Utility script 174 for lightweight workstation automation.
.DESCRIPTION
...
function RestoreFolderInventory174 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '174'
        ScriptName = 'RestoreFolderInventory174'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'RestoreFolderInventory174.log'
...
$result = RestoreFolderInventory174 -Destination $OutputPath
