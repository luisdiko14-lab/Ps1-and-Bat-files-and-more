<#
.SYNOPSIS
  Utility script 096 for lightweight workstation automation.
.DESCRIPTION
...
function CompareServiceStatus096 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '096'
        ScriptName = 'CompareServiceStatus096'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'CompareServiceStatus096.log'
...
$result = CompareServiceStatus096 -Destination $OutputPath
