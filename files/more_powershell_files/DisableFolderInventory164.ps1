<#
.SYNOPSIS
  Utility script 164 for lightweight workstation automation.
.DESCRIPTION
...
function DisableFolderInventory164 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '164'
        ScriptName = 'DisableFolderInventory164'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'DisableFolderInventory164.log'
...
$result = DisableFolderInventory164 -Destination $OutputPath
