<#
.SYNOPSIS
  Utility script 117 for lightweight workstation automation.
.DESCRIPTION
...
function FindNetworkLatency117 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '117'
        ScriptName = 'FindNetworkLatency117'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'FindNetworkLatency117.log'
...
$result = FindNetworkLatency117 -Destination $OutputPath
