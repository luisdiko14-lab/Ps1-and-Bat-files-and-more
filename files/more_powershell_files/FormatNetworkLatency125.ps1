<#
.SYNOPSIS
  Utility script 125 for lightweight workstation automation.
.DESCRIPTION
...
function FormatNetworkLatency125 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '125'
        ScriptName = 'FormatNetworkLatency125'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'FormatNetworkLatency125.log'
...
$result = FormatNetworkLatency125 -Destination $OutputPath
