<#
.SYNOPSIS
  Utility script 053 for lightweight workstation automation.
.DESCRIPTION
...
function TestTopProcesses053 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '053'
        ScriptName = 'TestTopProcesses053'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'TestTopProcesses053.log'
...
$result = TestTopProcesses053 -Destination $OutputPath
