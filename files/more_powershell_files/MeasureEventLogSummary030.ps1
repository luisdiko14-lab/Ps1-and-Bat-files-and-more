<#
.SYNOPSIS
  Utility script 030 for lightweight workstation automation.
.DESCRIPTION
...
function MeasureEventLogSummary030 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '030'
        ScriptName = 'MeasureEventLogSummary030'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'MeasureEventLogSummary030.log'
...
$result = MeasureEventLogSummary030 -Destination $OutputPath
