<#
.SYNOPSIS
  Utility script 076 for lightweight workstation automation.
.DESCRIPTION
...
function GetServiceStatus076 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '076'
        ScriptName = 'GetServiceStatus076'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'GetServiceStatus076.log'
...
$result = GetServiceStatus076 -Destination $OutputPath
