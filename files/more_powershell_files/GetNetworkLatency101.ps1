<#
.SYNOPSIS
  Utility script 101 for lightweight workstation automation.
.DESCRIPTION
...
function GetNetworkLatency101 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '101'
        ScriptName = 'GetNetworkLatency101'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'GetNetworkLatency101.log'
...
$result = GetNetworkLatency101 -Destination $OutputPath
