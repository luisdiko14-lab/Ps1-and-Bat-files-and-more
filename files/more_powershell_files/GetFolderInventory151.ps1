<#
.SYNOPSIS
  Utility script 151 for lightweight workstation automation.
.DESCRIPTION
...
function GetFolderInventory151 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '151'
        ScriptName = 'GetFolderInventory151'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'GetFolderInventory151.log'
...
$result = GetFolderInventory151 -Destination $OutputPath
