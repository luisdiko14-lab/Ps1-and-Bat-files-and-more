<#
.SYNOPSIS
  Utility script 234 for lightweight workstation automation.
.DESCRIPTION
...
function ReadCsvCleanup234 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '234'
        ScriptName = 'ReadCsvCleanup234'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ReadCsvCleanup234.log'
...
$result = ReadCsvCleanup234 -Destination $OutputPath
