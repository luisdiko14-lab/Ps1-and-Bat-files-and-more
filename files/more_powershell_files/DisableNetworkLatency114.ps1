<#
.SYNOPSIS
  Utility script 114 for lightweight workstation automation.
.DESCRIPTION
...
function DisableNetworkLatency114 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '114'
        ScriptName = 'DisableNetworkLatency114'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'DisableNetworkLatency114.log'
...
$result = DisableNetworkLatency114 -Destination $OutputPath
