<#
.SYNOPSIS
  Utility script 183 for lightweight workstation automation.
.DESCRIPTION
...
function WriteHashReport183 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '183'
        ScriptName = 'WriteHashReport183'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'WriteHashReport183.log'
...
$result = WriteHashReport183 -Destination $OutputPath
