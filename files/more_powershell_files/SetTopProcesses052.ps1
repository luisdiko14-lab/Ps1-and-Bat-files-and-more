<#
.SYNOPSIS
  Utility script 052 for lightweight workstation automation.
.DESCRIPTION
...
function SetTopProcesses052 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '052'
        ScriptName = 'SetTopProcesses052'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'SetTopProcesses052.log'
...
$result = SetTopProcesses052 -Destination $OutputPath
