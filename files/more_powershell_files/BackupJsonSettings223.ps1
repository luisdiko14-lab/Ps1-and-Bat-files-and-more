<#
.SYNOPSIS
  Utility script 223 for lightweight workstation automation.
.DESCRIPTION
...
function BackupJsonSettings223 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '223'
        ScriptName = 'BackupJsonSettings223'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'BackupJsonSettings223.log'
...
$result = BackupJsonSettings223 -Destination $OutputPath
