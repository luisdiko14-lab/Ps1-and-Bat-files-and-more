<#
.SYNOPSIS
  Utility script 235 for lightweight workstation automation.
.DESCRIPTION
...
function NewCsvCleanup235 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '235'
        ScriptName = 'NewCsvCleanup235'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'NewCsvCleanup235.log'
...
$result = NewCsvCleanup235 -Destination $OutputPath
