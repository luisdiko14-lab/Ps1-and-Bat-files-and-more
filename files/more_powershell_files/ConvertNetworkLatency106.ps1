<#
.SYNOPSIS
  Utility script 106 for lightweight workstation automation.
.DESCRIPTION
...
function ConvertNetworkLatency106 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '106'
        ScriptName = 'ConvertNetworkLatency106'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ConvertNetworkLatency106.log'
...
$result = ConvertNetworkLatency106 -Destination $OutputPath
