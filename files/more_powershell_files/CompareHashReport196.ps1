<#
.SYNOPSIS
  Utility script 196 for lightweight workstation automation.
.DESCRIPTION
...
function CompareHashReport196 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '196'
        ScriptName = 'CompareHashReport196'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'CompareHashReport196.log'
...
$result = CompareHashReport196 -Destination $OutputPath
