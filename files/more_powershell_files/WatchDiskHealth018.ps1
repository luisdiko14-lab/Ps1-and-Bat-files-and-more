<#
.SYNOPSIS
  Utility script 018 for lightweight workstation automation.
.DESCRIPTION
...
function WatchDiskHealth018 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '018'
        ScriptName = 'WatchDiskHealth018'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'WatchDiskHealth018.log'
...
$result = WatchDiskHealth018 -Destination $OutputPath
