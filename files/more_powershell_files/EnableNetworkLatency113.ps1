<#
.SYNOPSIS
  Utility script 113 for lightweight workstation automation.
.DESCRIPTION
...
function EnableNetworkLatency113 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '113'
        ScriptName = 'EnableNetworkLatency113'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'EnableNetworkLatency113.log'
...
$result = EnableNetworkLatency113 -Destination $OutputPath
