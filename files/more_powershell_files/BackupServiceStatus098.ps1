<#
.SYNOPSIS
  Utility script 098 for lightweight workstation automation.
.DESCRIPTION
...
function BackupServiceStatus098 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '098'
        ScriptName = 'BackupServiceStatus098'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'BackupServiceStatus098.log'
...
$result = BackupServiceStatus098 -Destination $OutputPath
