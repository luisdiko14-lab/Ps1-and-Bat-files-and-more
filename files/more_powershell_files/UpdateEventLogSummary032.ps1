<#
.SYNOPSIS
  Utility script 032 for lightweight workstation automation.
.DESCRIPTION
...
function UpdateEventLogSummary032 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '032'
        ScriptName = 'UpdateEventLogSummary032'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'UpdateEventLogSummary032.log'
...
$result = UpdateEventLogSummary032 -Destination $OutputPath
