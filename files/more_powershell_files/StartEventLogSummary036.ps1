<#
.SYNOPSIS
  Utility script 036 for lightweight workstation automation.
.DESCRIPTION
...
function StartEventLogSummary036 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '036'
        ScriptName = 'StartEventLogSummary036'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'StartEventLogSummary036.log'
...
$result = StartEventLogSummary036 -Destination $OutputPath
