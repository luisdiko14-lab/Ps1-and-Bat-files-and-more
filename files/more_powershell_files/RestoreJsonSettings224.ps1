<#
.SYNOPSIS
  Utility script 224 for lightweight workstation automation.
.DESCRIPTION
...
function RestoreJsonSettings224 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '224'
        ScriptName = 'RestoreJsonSettings224'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'RestoreJsonSettings224.log'
...
$result = RestoreJsonSettings224 -Destination $OutputPath
