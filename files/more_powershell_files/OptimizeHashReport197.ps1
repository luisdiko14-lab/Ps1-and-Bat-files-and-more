<#
.SYNOPSIS
  Utility script 197 for lightweight workstation automation.
.DESCRIPTION
...
function OptimizeHashReport197 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '197'
        ScriptName = 'OptimizeHashReport197'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'OptimizeHashReport197.log'
...
$result = OptimizeHashReport197 -Destination $OutputPath
