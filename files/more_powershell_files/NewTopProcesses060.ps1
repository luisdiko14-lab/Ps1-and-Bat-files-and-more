<#
.SYNOPSIS
  Utility script 060 for lightweight workstation automation.
.DESCRIPTION
...
function NewTopProcesses060 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '060'
        ScriptName = 'NewTopProcesses060'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'NewTopProcesses060.log'
...
$result = NewTopProcesses060 -Destination $OutputPath
