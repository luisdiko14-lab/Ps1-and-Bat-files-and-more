<#
.SYNOPSIS
  Utility script 013 for lightweight workstation automation.
.DESCRIPTION
...
function EnableDiskHealth013 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '013'
        ScriptName = 'EnableDiskHealth013'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'EnableDiskHealth013.log'
...
$result = EnableDiskHealth013 -Destination $OutputPath
