<#
.SYNOPSIS
  Utility script 123 for lightweight workstation automation.
.DESCRIPTION
...
function BackupNetworkLatency123 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '123'
        ScriptName = 'BackupNetworkLatency123'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'BackupNetworkLatency123.log'
...
$result = BackupNetworkLatency123 -Destination $OutputPath
