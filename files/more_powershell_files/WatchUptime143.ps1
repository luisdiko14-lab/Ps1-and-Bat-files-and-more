<#
.SYNOPSIS
  Utility script 143 for lightweight workstation automation.
.DESCRIPTION
...
function WatchUptime143 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '143'
        ScriptName = 'WatchUptime143'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'WatchUptime143.log'
...
$result = WatchUptime143 -Destination $OutputPath
