<#
.SYNOPSIS
  Utility script 105 for lightweight workstation automation.
.DESCRIPTION
...
function MeasureNetworkLatency105 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '105'
        ScriptName = 'MeasureNetworkLatency105'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'MeasureNetworkLatency105.log'
...
$result = MeasureNetworkLatency105 -Destination $OutputPath
