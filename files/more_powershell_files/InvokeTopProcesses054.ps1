<#
.SYNOPSIS
  Utility script 054 for lightweight workstation automation.
.DESCRIPTION
...
function InvokeTopProcesses054 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '054'
        ScriptName = 'InvokeTopProcesses054'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'InvokeTopProcesses054.log'
...
$result = InvokeTopProcesses054 -Destination $OutputPath
