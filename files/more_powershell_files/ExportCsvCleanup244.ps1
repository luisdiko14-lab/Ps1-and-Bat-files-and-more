<#
.SYNOPSIS
  Utility script 244 for lightweight workstation automation.
.DESCRIPTION
...
function ExportCsvCleanup244 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '244'
        ScriptName = 'ExportCsvCleanup244'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ExportCsvCleanup244.log'
...
$result = ExportCsvCleanup244 -Destination $OutputPath
