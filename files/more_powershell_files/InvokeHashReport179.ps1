<#
.SYNOPSIS
  Utility script 179 for lightweight workstation automation.
.DESCRIPTION
...
function InvokeHashReport179 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '179'
        ScriptName = 'InvokeHashReport179'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'InvokeHashReport179.log'
...
$result = InvokeHashReport179 -Destination $OutputPath
