<#
.SYNOPSIS
  Utility script 120 for lightweight workstation automation.
.DESCRIPTION
...
function ImportNetworkLatency120 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '120'
        ScriptName = 'ImportNetworkLatency120'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ImportNetworkLatency120.log'
...
$result = ImportNetworkLatency120 -Destination $OutputPath
