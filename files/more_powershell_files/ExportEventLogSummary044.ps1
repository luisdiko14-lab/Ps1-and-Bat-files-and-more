<#
.SYNOPSIS
  Utility script 044 for lightweight workstation automation.
.DESCRIPTION
...
function ExportEventLogSummary044 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '044'
        ScriptName = 'ExportEventLogSummary044'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ExportEventLogSummary044.log'
...
$result = ExportEventLogSummary044 -Destination $OutputPath
