<#
.SYNOPSIS
  Utility script 040 for lightweight workstation automation.
.DESCRIPTION
...
function RepairEventLogSummary040 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '040'
        ScriptName = 'RepairEventLogSummary040'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'RepairEventLogSummary040.log'
...
$result = RepairEventLogSummary040 -Destination $OutputPath
