<#
.SYNOPSIS
  Utility script 004 for lightweight workstation automation.
.DESCRIPTION
...
function InvokeDiskHealth004 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '004'
        ScriptName = 'InvokeDiskHealth004'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'InvokeDiskHealth004.log'
...
$result = InvokeDiskHealth004 -Destination $OutputPath
