<#
.SYNOPSIS
  Utility script 189 for lightweight workstation automation.
.DESCRIPTION
...
function DisableHashReport189 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '189'
        ScriptName = 'DisableHashReport189'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'DisableHashReport189.log'
...
$result = DisableHashReport189 -Destination $OutputPath
