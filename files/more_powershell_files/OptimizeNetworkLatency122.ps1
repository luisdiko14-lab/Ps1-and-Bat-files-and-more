<#
.SYNOPSIS
  Utility script 122 for lightweight workstation automation.
.DESCRIPTION
...
function OptimizeNetworkLatency122 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '122'
        ScriptName = 'OptimizeNetworkLatency122'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'OptimizeNetworkLatency122.log'
...
$result = OptimizeNetworkLatency122 -Destination $OutputPath
