<#
.SYNOPSIS
  Utility script 248 for lightweight workstation automation.
.DESCRIPTION
...
function BackupCsvCleanup248 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '248'
        ScriptName = 'BackupCsvCleanup248'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'BackupCsvCleanup248.log'
...
$result = BackupCsvCleanup248 -Destination $OutputPath
