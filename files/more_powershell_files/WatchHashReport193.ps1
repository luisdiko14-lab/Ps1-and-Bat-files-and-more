<#
.SYNOPSIS
  Utility script 193 for lightweight workstation automation.
.DESCRIPTION
...
function WatchHashReport193 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '193'
        ScriptName = 'WatchHashReport193'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'WatchHashReport193.log'
...
$result = WatchHashReport193 -Destination $OutputPath
