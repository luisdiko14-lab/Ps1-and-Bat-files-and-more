<#
.SYNOPSIS
  Utility script 187 for lightweight workstation automation.
.DESCRIPTION
...
function StopHashReport187 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '187'
        ScriptName = 'StopHashReport187'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'StopHashReport187.log'
...
$result = StopHashReport187 -Destination $OutputPath
