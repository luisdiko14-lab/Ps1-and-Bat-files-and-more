<#
.SYNOPSIS
  Utility script 017 for lightweight workstation automation.
.DESCRIPTION
...
function FindDiskHealth017 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '017'
        ScriptName = 'FindDiskHealth017'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'FindDiskHealth017.log'
...
$result = FindDiskHealth017 -Destination $OutputPath
