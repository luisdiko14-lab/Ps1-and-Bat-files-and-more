<#
.SYNOPSIS
  Utility script 091 for lightweight workstation automation.
.DESCRIPTION
...
function ClearServiceStatus091 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '091'
        ScriptName = 'ClearServiceStatus091'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ClearServiceStatus091.log'
...
$result = ClearServiceStatus091 -Destination $OutputPath
