<#
.SYNOPSIS
  Utility script 220 for lightweight workstation automation.
.DESCRIPTION
...
function ImportJsonSettings220 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '220'
        ScriptName = 'ImportJsonSettings220'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ImportJsonSettings220.log'
...
$result = ImportJsonSettings220 -Destination $OutputPath
