<#
.SYNOPSIS
  Utility script 152 for lightweight workstation automation.
.DESCRIPTION
...
function SetFolderInventory152 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '152'
        ScriptName = 'SetFolderInventory152'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'SetFolderInventory152.log'
...
$result = SetFolderInventory152 -Destination $OutputPath
