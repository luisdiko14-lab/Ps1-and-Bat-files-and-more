<#
.SYNOPSIS
  Utility script 059 for lightweight workstation automation.
.DESCRIPTION
...
function ReadTopProcesses059 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '059'
        ScriptName = 'ReadTopProcesses059'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ReadTopProcesses059.log'
...
$result = ReadTopProcesses059 -Destination $OutputPath
