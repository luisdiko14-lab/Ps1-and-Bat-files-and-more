<#
.SYNOPSIS
  Utility script 003 for lightweight workstation automation.
.DESCRIPTION
...
function TestDiskHealth003 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '003'
        ScriptName = 'TestDiskHealth003'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'TestDiskHealth003.log'
...
$result = TestDiskHealth003 -Destination $OutputPath
