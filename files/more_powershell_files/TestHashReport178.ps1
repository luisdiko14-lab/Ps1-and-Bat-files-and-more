<#
.SYNOPSIS
  Utility script 178 for lightweight workstation automation.
.DESCRIPTION
...
function TestHashReport178 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '178'
        ScriptName = 'TestHashReport178'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'TestHashReport178.log'
...
$result = TestHashReport178 -Destination $OutputPath
