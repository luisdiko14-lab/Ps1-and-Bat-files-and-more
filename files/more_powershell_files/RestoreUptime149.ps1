<#
.SYNOPSIS
  Utility script 149 for lightweight workstation automation.
.DESCRIPTION
...
function RestoreUptime149 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '149'
        ScriptName = 'RestoreUptime149'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'RestoreUptime149.log'
...
$result = RestoreUptime149 -Destination $OutputPath
