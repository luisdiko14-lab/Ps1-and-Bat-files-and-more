<#
.SYNOPSIS
  Utility script 081 for lightweight workstation automation.
.DESCRIPTION
...
function ConvertServiceStatus081 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '081'
        ScriptName = 'ConvertServiceStatus081'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ConvertServiceStatus081.log'
...
$result = ConvertServiceStatus081 -Destination $OutputPath
