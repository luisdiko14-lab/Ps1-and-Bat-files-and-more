<#
.SYNOPSIS
  Utility script 116 for lightweight workstation automation.
.DESCRIPTION
...
function ClearNetworkLatency116 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '116'
        ScriptName = 'ClearNetworkLatency116'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ClearNetworkLatency116.log'
...
$result = ClearNetworkLatency116 -Destination $OutputPath
