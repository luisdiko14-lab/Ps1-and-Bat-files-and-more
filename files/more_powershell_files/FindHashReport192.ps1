<#
.SYNOPSIS
  Utility script 192 for lightweight workstation automation.
.DESCRIPTION
...
function FindHashReport192 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '192'
        ScriptName = 'FindHashReport192'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'FindHashReport192.log'
...
$result = FindHashReport192 -Destination $OutputPath
