<#
.SYNOPSIS
  Utility script 156 for lightweight workstation automation.
.DESCRIPTION
...
function ConvertFolderInventory156 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '156'
        ScriptName = 'ConvertFolderInventory156'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ConvertFolderInventory156.log'
...
$result = ConvertFolderInventory156 -Destination $OutputPath
