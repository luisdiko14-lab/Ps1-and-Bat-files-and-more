<#
.SYNOPSIS
  Utility script 103 for lightweight workstation automation.
.DESCRIPTION
...
function TestNetworkLatency103 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '103'
        ScriptName = 'TestNetworkLatency103'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'TestNetworkLatency103.log'
...
$result = TestNetworkLatency103 -Destination $OutputPath
