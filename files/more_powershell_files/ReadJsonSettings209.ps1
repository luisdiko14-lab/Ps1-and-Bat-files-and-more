<#
.SYNOPSIS
  Utility script 209 for lightweight workstation automation.
.DESCRIPTION
...
function ReadJsonSettings209 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '209'
        ScriptName = 'ReadJsonSettings209'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ReadJsonSettings209.log'
...
$result = ReadJsonSettings209 -Destination $OutputPath
