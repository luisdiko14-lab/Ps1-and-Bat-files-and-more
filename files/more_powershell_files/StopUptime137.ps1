<#
.SYNOPSIS
  Utility script 137 for lightweight workstation automation.
.DESCRIPTION
...
function StopUptime137 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '137'
        ScriptName = 'StopUptime137'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'StopUptime137.log'
...
$result = StopUptime137 -Destination $OutputPath
