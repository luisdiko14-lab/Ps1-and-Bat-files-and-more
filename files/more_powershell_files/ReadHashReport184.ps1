<#
.SYNOPSIS
  Utility script 184 for lightweight workstation automation.
.DESCRIPTION
...
function ReadHashReport184 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '184'
        ScriptName = 'ReadHashReport184'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ReadHashReport184.log'
...
$result = ReadHashReport184 -Destination $OutputPath
