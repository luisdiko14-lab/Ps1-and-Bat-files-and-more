<#
.SYNOPSIS
  Utility script 087 for lightweight workstation automation.
.DESCRIPTION
...
function StopServiceStatus087 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '087'
        ScriptName = 'StopServiceStatus087'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'StopServiceStatus087.log'
...
$result = StopServiceStatus087 -Destination $OutputPath
