<#
.SYNOPSIS
  Utility script 208 for lightweight workstation automation.
.DESCRIPTION
...
function WriteJsonSettings208 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '208'
        ScriptName = 'WriteJsonSettings208'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'WriteJsonSettings208.log'
...
$result = WriteJsonSettings208 -Destination $OutputPath
