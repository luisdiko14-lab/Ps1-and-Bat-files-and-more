<#
.SYNOPSIS
  Utility script 124 for lightweight workstation automation.
.DESCRIPTION
...
function RestoreNetworkLatency124 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '124'
        ScriptName = 'RestoreNetworkLatency124'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'RestoreNetworkLatency124.log'
...
$result = RestoreNetworkLatency124 -Destination $OutputPath
