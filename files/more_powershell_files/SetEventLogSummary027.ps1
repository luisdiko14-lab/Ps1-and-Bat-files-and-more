<#
.SYNOPSIS
  Utility script 027 for lightweight workstation automation.
.DESCRIPTION
...
function SetEventLogSummary027 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '027'
        ScriptName = 'SetEventLogSummary027'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'SetEventLogSummary027.log'
...
$result = SetEventLogSummary027 -Destination $OutputPath
