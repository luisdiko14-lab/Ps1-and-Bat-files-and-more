<#
.SYNOPSIS
  Utility script 107 for lightweight workstation automation.
.DESCRIPTION
...
function UpdateNetworkLatency107 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '107'
        ScriptName = 'UpdateNetworkLatency107'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'UpdateNetworkLatency107.log'
...
$result = UpdateNetworkLatency107 -Destination $OutputPath
