<#
.SYNOPSIS
  Utility script 086 for lightweight workstation automation.
.DESCRIPTION
...
function StartServiceStatus086 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '086'
        ScriptName = 'StartServiceStatus086'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'StartServiceStatus086.log'
...
$result = StartServiceStatus086 -Destination $OutputPath
