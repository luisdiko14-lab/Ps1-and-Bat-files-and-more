<#
.SYNOPSIS
  Utility script 111 for lightweight workstation automation.
.DESCRIPTION
...
function StartNetworkLatency111 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '111'
        ScriptName = 'StartNetworkLatency111'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'StartNetworkLatency111.log'
...
$result = StartNetworkLatency111 -Destination $OutputPath
