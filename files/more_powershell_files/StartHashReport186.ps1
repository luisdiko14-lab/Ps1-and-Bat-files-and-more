<#
.SYNOPSIS
  Utility script 186 for lightweight workstation automation.
.DESCRIPTION
...
function StartHashReport186 {
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '186'
        ScriptName = 'StartHashReport186'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'StartHashReport186.log'
...
$result = StartHashReport186 -Destination $OutputPath
