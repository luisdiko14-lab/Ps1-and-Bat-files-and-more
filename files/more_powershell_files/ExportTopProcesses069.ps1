<#
.SYNOPSIS
  Utility script 069 for lightweight workstation automation.
.DESCRIPTION
...
function ExportTopProcesses069 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '069'
        ScriptName = 'ExportTopProcesses069'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'ExportTopProcesses069.log'
...
$result = ExportTopProcesses069 -Destination $OutputPath
