<#
.SYNOPSIS
  Utility script 163 for lightweight workstation automation.
.DESCRIPTION
...
function EnableFolderInventory163 {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Destination
    ...
    $record = [PSCustomObject]@{
        ScriptId   = '163'
        ScriptName = 'EnableFolderInventory163'
        RanAtUtc   = $now.ToUniversalTime().ToString('o')
        HostName   = $env:COMPUTERNAME
        UserName   = $env:USERNAME
    }

    $logFile = Join-Path -Path $Destination -ChildPath 'EnableFolderInventory163.log'
...
$result = EnableFolderInventory163 -Destination $OutputPath
